...
# Dice loss with weighted categorical cross-entropy loss
def dice_loss_with_CE(cls_weights, beta=1, smooth = 1e-5):
    ...
    def _dice_loss_with_CE(y_true, y_pred):
        ...
        tp = K.sum(y_true[...,:-1] * y_pred, axis=[0,1,2])  # true positive
        fp = K.sum(y_pred, axis=[0,1,2]) - tp   # false positive
        fn = K.sum(y_true[...,:-1], axis=[0,1,2]) - tp  # false negative

        # Dice loss
        score = ((1 + beta ** 2) * tp + smooth) / ((1 + beta ** 2) * tp + beta ** 2 * fn + fp + smooth) # Dice coefficient
